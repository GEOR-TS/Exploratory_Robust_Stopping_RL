...
class DeepBackward(nn.Module):
    def __init__(
        ...
        self.y_networks = nn.ModuleList([self._build_network(2, 1) for _ in range(n_time_steps)])
        self.z_networks = nn.ModuleList([self._build_network(1, 1) for _ in range(n_time_steps)])
    ...
    def _init_weights(self):
        for network in list(self.y_networks) + list(self.z_networks):
    ...
    def f_driver(self, t, x, y, z):
        ...
        log_1_exp = torch.logaddexp(torch.zeros_like(y), exponent)
        z_reg_term = self.small_g(z).unsqueeze(1)

        return f_x - discount_term + z_reg_term + penalty_term + log_1_exp

    def simulate_forward_process(self, batch_size):
        x = torch.zeros(batch_size, self.n_time_steps + 1, 1, device=self.device)
        x[:, 0, :] = self.x_0
        dw = torch.zeros(batch_size, self.n_time_steps, 1, device=self.device)

        for t in range(self.n_time_steps):
            dw[:, t, :] = np.sqrt(self.dt) * torch.randn(batch_size, 1, device=self.device)
            x[:, t + 1, :] = x[:, t, :] * torch.exp(
    ...
    def train(self, n_iterations=20000, batch_size=64, print_every=1000, evaluate_every=500):
        ...
        for it in tqdm(range(n_iterations)):
            ...
            for t in range(self.n_time_steps - 1, -1, -1):
                iteration_loss += self.train_step_backward(x, dw, t, self.optimizers[t])
